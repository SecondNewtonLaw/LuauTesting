 ...
 * */
void Module::Closures::ToLClosure(lua_State *L, int idx) {
    ...
    auto bytecode = execution->Compile(code);
    if (luau_load(L, utilities->RandomString(32).c_str(), bytecode.c_str(), bytecode.size(), -1) !=
        LUA_OK) {
...
int NewCClosureHandler(lua_State *L) {
    ...
    const char *error;
    const auto callResult = lua_pcall(L, argc, 0, 0);

    if (callResult && callResult != LUA_YIELD &&
        (error = luaL_optstring(L, -1, oxorany_pchar(L"")), 0 == std::strcmp(error, oxorany_pchar(L"attempt to yield across metamethod/C-call boundary"))))
